		...
		var defaultPrefs = {
			...
			uiStrings = Editor.uiStrings = {
				...
				notification: {
					...
					"noContentToFitTo":"无合适内容",
					"dupeLayerName":"此图层名已被占用",
					"enterUniqueLayerName":"请输入有效的图层名",
					"enterNewLayerName":"请输入新的图层名",
					"layerHasThatName":"图层已有此名称",
					"QmoveElemsToLayer":"将选中元素移至图层\"%s\"?",
					"QwantToClear":"确定要清空画板吗？\n这将清除您的修改！",
					"QwantToOpen":"确定要打开新地图吗？\n这将清除您的修改！",
					"QerrorsRevertToSource":"您的SVG源码中有拼写错误。\n是否恢复初始代码？",
					"QignoreSourceChanges":"是否忽略对SVG源码的修改？",
					"featNotSupported":"不支持的功能",
					"enterNewImgURL":"输入图片的URL",
					"defsFailOnSave": "NOTE: Due to a bug in your browser, this image may appear wrong (missing gradients or elements). It will however appear correct once actually saved.",
					"loadingImage":"Loading image, please wait...",
					"saveFromBrowser": "Select \"Save As...\" in your browser to save this image as a %s file.",
					"noteTheseIssues": "请注意以下几点： ",
					"unsavedChanges": "有未保存的改动",
					"enterNewLinkURL": "请输入超链接的URL",
					"errorLoadingSVG": "错误：无法保存SVG数据",
					"URLloadFail": "导入URL失败",
	...
	// 			var lang = ('lang' in curPrefs) ? curPrefs.lang : null;
			...
			$.svgIcons(curConfig.imgPath + 'svg_edit_icons.svg', {
				...
				resize: {
					...
					'.layer_button .svg_icon, #layerlist td.layervis .svg_icon': 24,
...
// 					}
			...
			(function() {
				...
				var dbox = function(type, msg, callback, defText) {
					...
					var ok = $('<input type="button" value="确认">').appendTo(btn_holder);
					...
					if(type == 'prompt') {
						if (msg==uiStrings.notification.enterNewImgURL) {
						var sltEmt = selectedElement.id;
						var ars = new Array('postcode','tel','address');
                            var ar ='<div id="attrBox_'+sltEmt+'"><form id="attrForm_'+sltEmt+'">'+ 
                                    '<table id="attributesBox_'+sltEmt+'" class="attrBoxClass">'+
                                        '<tr>'+
                                            '<th>属性</th>'+
                                            '<th>值</th>'+
                                        '</tr>'+
                                      '</table>'+
                                      '</form>'+
                                      '<div><input type="button" id="addAr" value="增加属性" style="margin-bottom:5px"></div><hr /></div>';
                            var tds = function(){
                                var tdstring ='';
                                for (var i = 0; i < ars.length; i++) {
                                    var tr ='<tr>'+
                                                '<td>'+ars[i]+'</td>'+
                                                '<td><input name="'+ars[i]+'" type="text" /></td>'+
                                            '</tr>';
                                    tdstring = tdstring + tr;
                                };
                                return tdstring;
                            };
                            $("#dialog_container").css({"height":"400px","top":"20%"});
                            $("#dialog_content.prompt").css("height","40px");
                            $(ar).prependTo(btn_holder);
                            $("#attributesBox_"+sltEmt).append(tds);
                            var newArNo = 0;
                            $("#addAr").click(function(){                                
                                var att = '<tr id="newAr'+newArNo+'">'+
                                            '<td><input type="text" /></td>'+
                                            '<td><input type="text" /></td>'+
                                            '<td><div onclick="javascript:delAr('+newArNo+')"><img src="images/cancel.png"/></div></td>'+
                                           '</tr>';
                                $("#attributesBox_"+sltEmt).append(att);
                                newArNo++;
                            });
						var input = $('<input id="imgUrl" type="text">').prependTo(btn_holder);
						if ($("#tools_image_show").attr("data-curopt")=="#tool_hotel"||$("#tools_image_show").attr("data-curopt")=="#tool_image") {
					...
					ok.click(function() {
						...
						if(selectedElement) var sltEmt = selectedElement.id;
						if(type == "prompt"){
                                for (var i = 0; i < newArNo; i++) {
                                    var newN = $("#newAr"+i+" td:first-child input").val();
                                    $("#newAr"+i+" td:nth-child(2) input").attr("name",newN);
                                    //alert(newN);
                                };
                                var formJson = getFormJson($('#attrForm_'+sltEmt));
                                //alert(formJson.Tel);
                            }
                        var formJson = type == "prompt" ? getFormJson($('#attrForm_'+sltEmt)) : true;
						if(callback) callback(resp,formJson);
					}).focus();
			...
			var saveHandler = function(window,svg) {
            	...
            	xmlHttp.open("POST","/GisWeb_GradleEclipse/save",true);
...
// 				$('#zoom').val(Math.round(zoomlevel*100));
			...
			var contextChanged = function(win, context) {
				...
				if(context) {
					...
					$(context).parentsUntil('#svgcontent > g').andSelf().each(function() {
						if(this.id) {
							var info = "";
                            for (var i = 1; i < context.childElementCount; i++) {
                            var af = context.childNodes[i].tagName;
                            var av = context.childNodes[i].innerHTML;
                            info = info + af+":"+av+"<br/>";
                            };
							str += ' > ' + this.id;
							if(this !== context) {
								...
								link_str += ' > ' + this.id + "<hr/><b>Attributes</b><br/>"+info;
							}
						}
					});
			...
			var setupFlyouts = function(holders) {
				$.each(holders, function(hold_sel, btn_opts) {
					...
					$(hold_sel).css({'left': pos.left+34, 'top': pos.top+50});
...
// 									key: btn.key,
	...
	// 							}
			...
			var setImageURL = Editor.setImageURL = function(url,formJson) {
				if(!url) url = default_img_url;

				svgCanvas.setImageURL(url,formJson);
				$('#image_url').val(url);
...
// 					}
					...
					menu_items.disableContextMenuItems('#edit,#delete,#cut,#copy,#group,#ungroup,#move_front,#move_up,#move_down,#move_back');
				}
				...
				if ( (elem && !is_node)	|| multiselected) {
					...
					canv_menu.enableContextMenuItems('#edit,#delete,#cut,#copy,#move_front,#move_up,#move_down,#move_back');
				}
				else {
...
// 					console.log('cm', $(evt.target).closest('.contextMenu'));
			...
			*/
			...
			//revise
			var clickHotel = function(){
				if (toolButtonClick('#tool_hotel')){
					svgCanvas.setMode('hotel');
				}
			}

			var clickUser = function(){
				if (toolButtonClick('#tool_user')){
					svgCanvas.setMode('user');
				}
			}
			...
			var editSelected = function(){
				if (selectedElement != null || multiselected) {
					svgCanvas.editSelectedElements();
				} 
			}
...
// 					}
			...
			function promptImgURL() {
				...
				$.prompt(uiStrings.notification.enterNewImgURL, curhref, function(url,formJson) {
					if(url) setImageURL(url,formJson);
				});
			}
...
// 				var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
			...
			$('#tool_help').click(function(){
				$.alert(uiStrings.notification.helpInfo,"", function() {
				});
			});
...
// 			});
		...
		Editor.loadFromLayer = function(){
			Editor.ready(function(){
				var layerInfo = [];
				var strSVGHead = '<svg width="8192" height="7167.999999999999" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">';
				var strSVGTail = '</svg>';
				var content='';
				var loadLayer = function(layer){
					...
					xmlHttp.onreadystatechange = function() {
						...
						if (xmlHttp.readyState==4 && xmlHttp.status==200)
					    {
							var tempcontent=xmlHttp.responseText;
							//console.log(content);
					...
					xmlHttp.open("GET", "/GisWeb_GradleEclipse/resource/svg/block/"+layer+"/"+1+"/"+1, true);
					xmlHttp.send();
				};
