...
// 					console.log('s',[start_x, start_y]);
			...
			case "image":
				...
				setHref(newImage, 'images/132placeholder.svg');
			...
			case "hotel":
				...
				setHref(newImage, 'images/134house.svg');
				preventClickDefault(newImage);
				break;
			case "user":
				started = true;
...
// 							call("selected", [selected]);
			...
			case "hotel":
				var attrs = $(element).attr(["width", "height"]);
				keep = (attrs.width != 0 || attrs.height != 0) || current_mode === "`";
				break;
			case "user":
				var attrs = $(element).attr(["width", "height"]);
				// Image should be kept regardless of size (use inherit dimensions later)
				keep = (attrs.width != 0 || attrs.height != 0) || current_mode === "user";
				break;
			case "circle":
...
this.setImageURL = function(val,formJson) {
	var elem = selectedElements[0];
	if(formJson){
            (elem).insertAdjacentHTML("beforeBegin","<g id='"+"g_"+elem.id+"'>");
            $("#g_"+elem.id).append($("#"+elem.id));
            for(var key in formJson){
                if(key == "postcode"||key == "tel"||key == "address"){
                    var attrVal = "<"+key+">"+eval("formJson."+key)+"</"+key+">";
                    var attrTag = document.getElementById("g_"+elem.id);
                    attrTag.insertAdjacentHTML("beforeEnd",attrVal);
                }
                else{
                    elem.setAttribute(key, eval("formJson."+key));
                }
            }
        }
...
// history stack
this.editSelectedElements = function(){
        //alert("edit");
        //alert(L.toString());
        var L = selectedElements[0];
        var sltEmt = selectedElements[0].firstChild.id;
        $("#editBox").show();
        /*if ($("#editAttr div:first-child")) {
        	$("#editAttr div:first-child").replaceAll($("#attrBox_"+sltEmt));
    	}
    	else{*/
    		$("#editAttr").append($("#attrBox_"+sltEmt));
    	/*};*/
        $("#editButt button:first-child").click(function(){
            for (var i = 0; i < $("#attributesBox_"+sltEmt+" tr input").length-1; i++) {
                if(i < $(L).children().length-1){
                    $(L).children()[i+1].innerHTML = $("#attributesBox_"+sltEmt+" tr input")[i].value;
                }
                 else{
                    var tgN = $("#attributesBox_"+sltEmt+" td input")[i].value;
                    var tgV = $("#attributesBox_"+sltEmt+" td input")[i+1].value;
                    $(L).children()[0].setAttribute(tgN,tgV);
                }
            };           
